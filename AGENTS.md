...
* **Compare:** `pages/1_Overview.py` is the shared view for showing teammates what each institute chose — the live current-session charts on top, then a saved-institutes overview table plus a multi-institute radar overlay below, so several people assessing different institutes (or the same institute at different times) can see and compare results without needing a database.
