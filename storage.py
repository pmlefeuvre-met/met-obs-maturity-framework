...
The confirm-before-overwrite prompt lives in app.py; this module just does
the read/write. Plain YAML (not a DB) so saved assessments stay human-
...
to one institute) and restored on read, so app.py's session_state keys don't
need to change.
...
def _read(path: str) -> SavedAssessment | None:
    ...
    return SavedAssessment(
        ...
        answers=_add_prefix(institute, doc.get("answers", {})),
    )
